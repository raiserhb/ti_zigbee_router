...
extern void Uart1_WriteRquest(uint8_t *buf, uint8_t len);
extern void Uart3_WriteRquest(uint8_t *buf, uint8_t len);
...
/**
  ...
  * @retval None
  */
extern uint8_t ReceiveSate_Usarty;
extern uint8_t ReceiveSate_Usartz;
void USARTy_IRQHandler(void)//usart1
{
	//uint8_t uart1_rdata = 0;
	uint8_t Clear = Clear;
  if(USART_GetITStatus(USARTy, USART_IT_RXNE) != RESET)
  {
    /* Read one byte from the receive data register */
    RxBuffer1[RxCounter1++] = USART_ReceiveData(USARTy);
	}else if(USART_GetITStatus(USARTy,USART_IT_IDLE) != RESET)
	{	
		Clear=USARTy->SR;
		Clear=USARTy->DR;
		ReceiveSate_Usarty = 1;
  }
...
/**
  ...
  * @retval None
  */
void USARTz_IRQHandler(void)//usrat3
{
	//uint8_t uart3_rdata = 0;
	uint8_t Clear=Clear;
  ...
  if(USART_GetITStatus(USARTz, USART_IT_RXNE) != RESET)
  {
	...
	}else if(USART_GetITStatus(USARTz,USART_IT_IDLE) != RESET)
	{
		Clear=USARTz->SR;
		Clear=USARTz->DR;
		ReceiveSate_Usartz = 1;
  }
}
