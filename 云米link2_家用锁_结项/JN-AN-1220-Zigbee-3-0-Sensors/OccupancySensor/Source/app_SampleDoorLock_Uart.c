...
PUBLIC volatile uint8 RxSerialMsg[80];	//Ricky ���ڴ�����������
...
void App_Handle_User_Write_Attribute(uint8 *palyload, uint16 size)//moons
{
	...
	if(attributeId == E_CLD_BAS_ATTR_ID_TRANSPRT_TRANS_CNT)
	{
		...
		if(GateWayCacheAllCnt != 0)
		{
			...
			ZTIMER_eStart(u8TimerCheckGetCacheKeepRun, ZTIMER_TIME_SEC(5));// and start time out timer
		}
		else
		{
			//�����׶ε�1����data request����������ϡ�Զ�̿�����1����data requestҲ����������ϡ�do not change data request when OTA switch to new image 
			if(u8JoinNetworkFlowPath == 0 && App_SampleDoorLockWakeUpTimeOutDecide() && !bOTA_IsWaitToUpgrade())
			{
				if(bPeriodicEventWakeup == TRUE)//��������Ե����������Ļ��Ϳ���ֱ�������ˣ�����ǰ������Ѿ���ʱһ�������
				{
					App_PollRateCnt = 0;
				}
			}

			requestCacheStatus = E_NoWaitAnything;// reset get cache statu, moon
			ZTIMER_eStop(u8TimerCheckGetCacheKeepRun);
			ZTIMER_eStart(u8TimerCheckGetCacheKeepRun, ZTIMER_TIME_SEC(3));//moon
			
			DBG_vPrintf(TRUE, "\n\n Gateway no Cache Data!\n\n");
		}
	#if 0
			DBG_vPrintf(TRACE_WR_ATTR, "\n eAttributeDataType %0x u16AttributeId %0x",eAttributeDataType,u16AttributeId);
...
{	
	...
	if(p[0] == 0xFF && p[1] == 0xFD)// for all other command�� ������һЩ���������߲���Ҫ������ָ�����кŵ�͸�������������
	{
	...
	else if(p[0] == 0xFF && p[1] == 0xFC)// for password operation �� ���ר�����ڷ������·����룬��������ָ������
	{
		if(size > 20)
		{
	...
	else if(p[0] == 0xFF && p[1] == 0xFB)// "FF FB" + "Version" + "AppCmdId" + "commandId"�������·����룬������Ҫ������ָ�����кŵ�ָ�����ָ����ID����ڻ����ServerAddDoorUserSeed
	{//�����ɷ������·�������ָ���Ҫ������ָ�����У�����Ӧ�������ʱ�������˵�ͬ�¸�����֪��Ӧ�������һ����ָ��
		if(size > 4)
		{
			pwdVersion = p[2];// 
			AppCmdId = p[3];
...
void App_SendDataImmediatelyOrSaveIntoCacheBuffer(uint8 AppCmdId, uint8 commandId, uint8 size, uint8 *p)//E_CLD_DOOR_LOCK_CMD_SET_YEAR_DAY_SCHEDULE
{
	...
	else
	{
		...
		for(cnt = 0; cnt < SERVER_WA_BUFFFERSIZE; cnt++)
		{
			if(u8ServerWriteAttributData[ cnt ][0] == IS_EMPTY)// u8ServerWriteAttributData[i][0] is empty so save data into buffer
			{
				...
				if(commandId == PwdOperation || commandId >= ServerAddDoorUserSeed)// PwdOperation��ServerAddDoorUserSeed �⼸�����˷�����ָ�����к�
				{
					u8ServerWriteAttributData[ cnt ][2] = AppCmdId;
					Cacheoffset = 3;					
					//u8ServerWriteAttributData[ cnt ][3 ~ end] is data
				}
				else// ��Щ��û�мӷ�����ָ�����кŵģ������޷�������ָ��Ψһ��ʶ�����ж�
				{
					Cacheoffset = 2;
	...
	if(commandId == PwdOperation || commandId >= ServerAddDoorUserSeed)//��ʱδ��ɷ�����ָ��ID
	{
		TxSerialMsg[4]	= 0;
		TxSerialMsg[5]	= 0;
		TxSerialMsg[6]	= 0;
