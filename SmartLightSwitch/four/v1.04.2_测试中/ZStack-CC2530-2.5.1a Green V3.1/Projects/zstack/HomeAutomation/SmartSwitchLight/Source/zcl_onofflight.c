...
uint16 zclOnOffLight_event_loop( uint8 task_id, uint16 events )
{
  ...
  if (events & ZCL_DEVICE_TOUCHPANEL_EVENT) //去抖200ms
  {
    if( (P0_0 == 0) || (P0_1 == 0) || (P1_0 == 0) || (P1_2 == 0)){
      osal_start_timerEx(zclOnOffLight_TaskID,
                         ZCL_DEVICE_DONEINTERRUPT_EVENT,
                         100);
    }else{
      P0IEN |= BV(0);
      P0IEN |= BV(1);
  ...
  if(events & ZCL_DEVICE_DONEINTERRUPT_EVENT) 
  {
    zclTouchPanelEvent();
    //return (events ^ ZCL_DEVICE_DONEINTERRUPT_EVENT);
  }
  ...
  case ZCL_CLUSTER_ID_GEN_ON_OFF:
    {
      if(pkt->cmd.Data[3]== DEVICE_All || pkt->cmd.Data[3]== DEVICE_LIGHT)
      {
        ...
        zclOnOffLight1_OnOff = pkt->cmd.Data[2];
        zclOnOffLight2_OnOff = pkt->cmd.Data[2];
        zclOnOffLight3_OnOff = pkt->cmd.Data[2];
        
        osal_start_timerEx(zclOnOffLight_TaskID, 
                           ZCL_DEVICE_SEND_ONOFF_STATE_EVENT,
                           ZCL_DEVICE_SEND_INFO_TIME_2S);
      }
    }
    break;
  default:
