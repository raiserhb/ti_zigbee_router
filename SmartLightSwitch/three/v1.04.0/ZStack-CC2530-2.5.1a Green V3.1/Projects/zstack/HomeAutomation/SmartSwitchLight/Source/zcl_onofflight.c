...
uint8 ledcontrol = 0;//0表示指示灯闪烁 1表示不闪烁
...
uint16 zclOnOffLight_event_loop( uint8 task_id, uint16 events )
{
  ...
  if(events & ZCL_DEVICE_INTERRPUT_DONE_EVENT) {
    zclTouchPanelEvent();
  ...
  if (events & ZCL_DEVICE_TOUCHPANEL_EVENT) {
    if((P0_0 == 0)||(P0_1 == 0)||(P1_0 == 0)){
      osal_start_timerEx(zclOnOffLight_TaskID,
                         ZCL_DEVICE_INTERRPUT_DONE_EVENT,
                         100);
    }else{
      P0IEN |= BV(0);
      P0IEN |= BV(1);
