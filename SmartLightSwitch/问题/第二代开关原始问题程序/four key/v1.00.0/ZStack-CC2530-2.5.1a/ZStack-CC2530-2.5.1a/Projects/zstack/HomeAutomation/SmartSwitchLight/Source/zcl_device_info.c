...
afAddrType_t zcl_HeartBeatCoord_nwkAddr;
...
void zclDeviceInfoInit(void)
{
  ...
  zcl_HeartBeatCoord_nwkAddr.addrMode = afAddr16Bit;
  zcl_HeartBeatCoord_nwkAddr.addr.shortAddr = 0x0000;
  zcl_HeartBeatCoord_nwkAddr.endPoint = 0xF0;
}
...
static void zclActiveEPRsp(void)
{
  ...
  srcAddr.addrMode = AddrBroadcast;
  srcAddr.addr.shortAddr = 0xFFFD;
...
static void zclSimpleDescRsp(uint16 endpoint)
{
  ...
  imMsg->srcAddr.addrMode = AddrBroadcast;
  imMsg->srcAddr.addr.shortAddr = 0xFFFD;
...
afStatus_t zclSendHeartbeat(void)
{
  ...
  stat = AF_DataRequest(&zcl_HeartBeatCoord_nwkAddr, 
                        &zclHeartbeatDesc,
...
void zclSendDeviceInfo(void)
{
  ...
  case 2:
    ...
    break;
  case 3:
    zclSimpleDescRsp(ONOFFLIGHT2_ENDPOINT);
    break;
  case 4:
    zclSimpleDescRsp(ONOFFLIGHT3_ENDPOINT);
    break;
  default:
    gDeviceInfoSendStatus = 0;
    osal_start_timerEx(ZCL_TASK_ID, 
